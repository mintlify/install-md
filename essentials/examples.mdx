...
Integrating an SDK into an existing codebase:
...
- Look for package.json, next.config.js, or other framework config files
- Identify if this is React, Next.js, Vue, or another framework
- Check if the project uses TypeScript by looking for tsconfig.json

Detect the package manager being used (check for package-lock.json, yarn.lock,
...
- App Router: Look for app/layout.tsx to modify
- Pages Router: Look for pages/_app.tsx to modify

Identify existing files that should be modified:
- Check for provider files (providers.tsx, context/, etc.)
- Look for app initialization files
- Prefer modifying existing files over creating new ones

Add PostHog initialization code:
- Import posthog-js at the top of the file
- Initialize PostHog with API key from environment variables
- For React: Wrap the application with PostHogProvider if needed
...
Verify the integration by checking the browser console for PostHog
initialization messages when running the development server.

Preserve all existing code formatting, style, and import patterns throughout.
```
