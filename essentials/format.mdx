...
Detect my project type by examining package.json and the directory structure
to identify the framework (React, Next.js, Vue, etc.) and whether TypeScript
is being used.

Detect which package manager is being used by looking for lock files
(package-lock.json, yarn.lock, pnpm-lock.yaml, bun.lockb).

Install the PostHog SDK with the appropriate command:
- npm: npm install posthog-js
- yarn: yarn add posthog-js
- pnpm: pnpm add posthog-js
- bun: bun add posthog-js

Identify existing files that should be modified, such as provider files,
layout files, or app entry points. Prefer modifying existing files over
creating new ones.

Add PostHog initialization code following the project's existing code style,
import patterns, and formatting. Configure it to use environment variables
for the API key and host.

Create or update .env.local with the necessary PostHog configuration values.
```
...
> MyAnalytics is an analytics SDK for tracking user behavior.

Install MyAnalytics into my project.

Detect my project framework by examining the repository structure and
configuration files. Identify if using React, Next.js, Vue, or another
framework, and whether TypeScript is enabled.

Detect the package manager by checking for lock files (package-lock.json,
yarn.lock, pnpm-lock.yaml, bun.lockb).

Install the SDK with the appropriate command:
- npm: npm install myanalytics
- yarn: yarn add myanalytics
- pnpm: pnpm add myanalytics
- bun: bun add myanalytics

Identify existing files to modify (providers, app entry points, layout files).
Prefer modifying existing files over creating new ones to match project patterns.

Add initialization code following the project's existing code style and import
patterns. Configure using environment variables for sensitive values.

Verify the integration works by checking for initialization in the console.
```
